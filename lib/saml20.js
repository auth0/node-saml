...
function createAssertion(options, strategies, callback) {
  ...
  var now = moment.utc();
  var maxClockSkew = options.maxClockSkewInSeconds || 0
  var notBefore = now.clone().subtract(maxClockSkew, 'seconds')
  ...
  if (options.lifetimeInSeconds) {
    conditions[0].setAttribute('NotBefore', notBefore.format('YYYY-MM-DDTHH:mm:ss.SSS[Z]'));
