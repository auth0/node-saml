...
describe('saml 1.1', function () {
  ...
  it('should set unspecified AuthenticationMethod attribute', function () {
    var options = {
      cert: fs.readFileSync(__dirname + '/test-auth0.pem'),
      key: fs.readFileSync(__dirname + '/test-auth0.key'),
      unspecifiedAuthenticationMethod: true
    };

    var signedAssertion = saml11.create(options);
    var authenticationStatement = utils.getAuthenticationStatement(signedAssertion);
    assert.equal('urn:oasis:names:tc:SAML:1.0:am:unspecified', authenticationStatement.getAttribute('AuthenticationMethod'));
  });
